...
export function registerMatterTools(server: McpServer): void {
  ...
  registerTool(server, {
    ...
    schema: {
      ...
      practice_area_id: z.number().int().optional().describe("From list-practice-areas."),
      sub_status_id: z.number().int().optional().describe("From list-sub-statuses."),
      contact_id: z.number().int().optional().describe("Attach an existing contact. Overrides match_contact_by."),
      match_contact_by: z
        .enum(["email", "name", "phone"])
        .optional()
        .describe("Reuse an existing contact matched on this attribute instead of creating a duplicate."),
      company_id: z.number().int().optional().describe("Create as a company matter for this company."),
  ...
  registerTool(server, {
    ...
    schema: {
      ...
      sub_status_id: z.number().int().optional(),
      assigned_staff_ids: z.array(z.number().int()).optional().describe("User IDs from list-users."),
